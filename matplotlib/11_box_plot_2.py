...
plt.boxplot([sales,engineering,hr,marketing],patch_artist=True)
plt.title('Employees salaries by department')
plt.xlabel('Department', fontsize=12, fontweight='bold')
plt.ylabel('Monthly Salary (in $1000s)', fontsize=12, fontweight='bold')
plt.grid(axis='y',alpha=0.5)
company_avg=55
plt.axhline(company_avg,linestyle='--',color='red',label='average')
plt.legend()
plt.show()